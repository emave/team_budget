...
export function DuesForm({ currentCents, currency }: { currentCents: number; currency: string }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 12, maxWidth: 360 }}>
      ...
      {run.isSuccess && <div style={{ color: '#16a34a' }}>Generated {(run.data as { createdCount: number; period: string }).createdCount} dues for {(run.data as { createdCount: number; period: string }).period}.</div>}
    </div>
  );
}
